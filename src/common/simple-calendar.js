...
var LunarHelp = function() {
    ...
    function LunarHelp(year, month, day) {
        ...
        // 保留农历信息
        this.lunarInfo = new Array(0x04bd8,0x04ae0,0x0a570,0x054d5,0x0d260,0x0d950,0x16554,0x056a0,0x09ad0,0x055d2,//1900-1909
        ...
        var i,
            leap = 0, 
            temp = 0; // 用于保留农历年一年的总天数

        var offset = (Date.UTC(date.getFullYear(),date.getMonth(),date.getDate()) - Date.UTC(1900,0,31))/86400000; // 计算查询的这天与1900年1月31日相差的天数

        //计算该年的农历年数
        for(i=1900; i<2101 && offset>0; i++) {
            temp = this.lYearDays(i); // 该年农历一年的天数
            offset -= temp;
        }
        if( offset<0 ) {
            offset += temp; i--; // 
...
export var SimpleCalendar = function() {
    //构造函数
    ...
    _createClass(SimpleCalendar, [{
        ...
        value: function create() {
            ...
            root.innerHTML = '<div class="sc-header"> </div> <div class="sc-body"> </div> <div class="lzc-week"> </div>';
            ...
            var lzcweek = root.querySelector('.lzc-week');
            //actions
            ...
            for (var i = 0; i < 6; i++) {
                lzcweek.innerHTML = lzcweek.innerHTML + '<div class="lzcweek-item' + i + '">' + '</div>';
            }
            for (var i = 0; i < 42; i++) {
        ...
        value: function addData(year, month) {
            ...
            // 填充周期格子
            if ((month == 1) && (pass_week > 1)) {
                this.container.querySelector('.lzcweek-item0').classList.add('lzcweek-item'); //添加样式类名
                this.container.querySelector('.lzcweek-item0').innerHTML = pass_week + 1;
                if (rownum != 6) {
                    this.container.querySelector('.lzcweek-item5').classList.remove('lzcweek-item');
                    this.container.querySelector('.lzcweek-item5').innerHTML = '';
                };
                for (var n = 1; n < rownum; n++) {
                    this.container.querySelector('.lzcweek-item' + n).classList.add('lzcweek-item');
                    this.container.querySelector('.lzcweek-item' + n).innerHTML = n;
                };
            } else {
                if (rownum != 6) {
                    this.container.querySelector('.lzcweek-item5').classList.remove('lzcweek-item');
                ...
                for (var n = 0; n < rownum; n++) {
                    this.container.querySelector('.lzcweek-item' + n).classList.add('lzcweek-item');
                    this.container.querySelector('.lzcweek-item' + n).innerHTML = pass_week + n +1;
                };
            }
            //对每一个格子遍历
            for (var i = 0; i < 42; i++) {
                ...
                if (writemonth != month) {
                    ...
                    daysElement[i].classList.remove('sc-othermenth');
                }
                if (i < 35 || itemsnum == 42) {
                    daysElement[i].classList.remove('sc-festival');
                    daysElement[i].classList.remove('sc-vocation');
                    daysElement[i].classList.add("sc-init-item");
                    daysElement[i].querySelector('.day').innerHTML = writeday;
                } else {
                    daysElement[i].classList.remove("sc-init-item");
                    daysElement[i].querySelector('.day').innerHTML = '';
                }
                ...
                if (this.tyear == writeyear && this.tday == writeday && this.tmonth == writemonth) {
                    ...
                    daysElement[i].classList.remove("sc-today");
                }
            }
        }
        ...
        value: function updateFestival(year, month) {
            var options = this._options; // 软件程序配置
            var daysElement = this.arrayfrom(this.container.querySelectorAll('.sc-item'));
            var currentmonth = month - 1;
            //取得节日数据
            ...
            daysElement.forEach(function(v, i) {
                ...
                if (options.showSolarTerm) {
                    if (solarTermdata[currentmonth + '-' + day]) {
                        // alert(lunar.month+ '-' + lunar.day + lunardata[lunar.month + '-' + lunar.day]);
                        // alert(lunardata['1-1'])
        ...
        value: function updateEvent() {
            ...
            daysElement.forEach(function(v, i) {
                ...
                v.onclick = function() {
                    ...
                    /* 更新点击日期 */
                    $(".event_title").text(myCalendar.getSelectedDay() + ' 我的日程')
                };
            });
            ...
            yearadd.onclick = function() {
                ...
                calendar.update(selectMonth.value, currentyear);
            };
...
SimpleCalendar.prototype.languageData = {
    ...
    vocation: {
        ...
        data_2017: ['1-1','1-27','1-28','1-29','1-30','1-31','2-1','2-2','4-2','4-3','4-4','5-1','5-28','5-29','5-30','5-30','10-1','10-2','10-3','10-4','10-5','10-6','10-7','10-8'],
